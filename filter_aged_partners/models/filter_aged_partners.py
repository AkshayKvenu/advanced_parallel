...
class FilterAgedPartner(models.TransientModel):
    ...
    def _print_report(self, data):
        ...
        return self.env.ref('accounting_pdf_reports.action_report_aged_partner_balance').report_action(self, data=data)

#         return self.env['report.accounting_pdf_reports.report_agedpartnerbalance']._get_report_values('account.report_agedpartnerbalance',
#                                                                            data=data)
...
# #         return self.env.ref('report.account.report_agedpartnerbalance').report_action(self, data=data)
#         return self.env['report.accounting_pdf_reports.report_agedpartnerbalance'].with_context(landscape=True)._get_report_values('account.report_agedpartnerbalance',
#                                                                            data=data)
    ...
    def _get_partner_move_lines(self, account_type, date_from, target_move, period_length,customer):
        ...
        for line in self.env['account.move.line'].browse(aml_ids):
            ...
            for partial_line in line.matched_debit_ids:
                if partial_line.create_date <= datetime.strptime(date_from, "%Y-%m-%d"):
        ...
        for i in range(5):
            ...
            for line in self.env['account.move.line'].browse(aml_ids):
                ...
                if line.balance == 0:
                    continue
                for partial_line in line.matched_debit_ids:
                    print('qqqqqqqqqqqqqqqqqqqqqqqq',partial_line.create_date,date_from)
                    if partial_line.create_date <= datetime.strptime(date_from, "%Y-%m-%d"):
                        line_amount += partial_line.amount
                for partial_line in line.matched_credit_ids:
                    if partial_line.create_date <= datetime.strptime(date_from, "%Y-%m-%d"):
...
#         print("aaaaaaaaaaaa",data.get('form'),self.env.context)
        if not data.get('form') or not self.env.context.get('active_model') or not self.env.context.get('active_id'):
            raise UserError(_("Form content is missing, this report cannot be printedsss."))
