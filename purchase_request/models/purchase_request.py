...
class PurchaseRequest(models.Model):
    ...
    assigned_to = fields.Many2one(
        ...
        domain=lambda self: [('groups_id', 'in', self.env.ref(
            'purchase_request.group_purchase_request_approver').id)]
    )
    ...
    purchase_line_compare = fields.Char()
    
    login_user = fields.Boolean('Login User',compute='check_value')
    
    @api.multi 
    def check_value(self):
        for result in self:
            user = self.env.user
            if user.has_group('purchase_request.group_purchase_request_manager') or result.assigned_to.id == user.id:
                result.login_user = True
            else:
                result.login_user = False
    ...
    def create(self, vals):
        ...
        follow_act = self.env.ref('purchase_request.add_follower_purchase_request')
        act_run = follow_act.with_context({'active_id': request.id}).run()        
        return request
    ...
    def write(self, vals):
        ...
        for request in self:
            if vals.get('assigned_to'):
                ...
                request.message_subscribe(partner_ids=[partner_id])
#         follow_act = self.env.ref('purchase_request.add_follower_purchase_request')
#         act_run = follow_act.with_context({'active_id': self.id}).run()        
        return res
    ...
    def button_approved(self):
        template = self.env.ref('purchase_request.email_template_approved_followers')
