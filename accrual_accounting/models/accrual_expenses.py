...
class AccrualAccounting(models.Model):
    _name = 'accrual.expense.accounting'

    parent_id = fields.Many2one('accrual.expense.accounting',default = False)
    ...
    deferred_amount = fields.Float("Deferred Expense Amount")
    ...
    def _posted_gross_increase_count(self):
        for asset in self:
            res = self.env['accrual.expense.accounting'].search_count([('parent_id', '=', asset.id)])
            asset.gross_increase_count = res or 0
    
    @api.multi
    def _posted_entry_count(self):
        for asset in self:
            res = self.env['account.move'].search_count([('accrual_account_expense_id', '=', asset.id), ('state', '=', 'posted')])
...
#     delivery_count = fields.Integer("Posted Entries",default="5")
    ...
    deferred_amount_id = fields.Many2one('account.account',"Deferred Expense Amount")
    expense_account_id = fields.Many2one('account.account',"Expense Amount")
    depreciation_move_ids = fields.One2many('account.move','accrual_account_expense_id', string="Depreciation Lines")
    original_move_line_ids = fields.One2many(comodel_name='account.move.line',inverse_name='move_line_expense_id', string="Journal Items")
...
#                 vals['deferred_amount'] = vals['residual_amount'] +self.gross_increase_value
    ...
    def gross_entries(self):
        asset_ids = []
        for asset in self.env['accrual.expense.accounting'].search([('parent_id', '=', self.id)]):
            asset_ids.append(asset.id)
        return {
            'name': _('Gross Increase'),
            'view_type': 'form',
            'view_mode': 'tree,form',
            'res_model': 'accrual.expense.accounting',
            'view_id': False,
    ...
    def action_move_create(self):
        ...
        if not self.deferred_amount_id:
            raise ValidationError(_("Field Deferred expense amount is not defined"))
        ...
        self.env['account.move'].search([('accrual_account_expense_id', '=',self.id)]).unlink()
...
#     
#     accrual_account_id = fields.Many2one('accrual.expense.accounting',"Accrual account")
