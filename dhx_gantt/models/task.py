...
class stock(models.Model):
    ...
    def button_validate(self):
        result = super(stock, self).button_validate()
        for rec in self.move_ids_without_package:
            for line in rec.move_line_ids:
                print("sssssssssss",self.env['project.task'].search([('stock_move_line_id', '=', line.id)]))
                search_task=self.env['project.task'].search([('stock_move_line_id', '=', line.id)])
                for task in search_task:
                    task.date_start = self.date_done
        return result
...
class Task(models.Model):
    ...
    def create(self, vals):
        ...
        if result.sale_line_id.rental_uom_id.period_type == 'month':
            result.planned_duration=result.sale_line_id.rental_qty * 30
        elif result.sale_line_id.rental_uom_id.period_type == 'day':
            result.planned_duration=result.sale_line_id.rental_qty
        return result
    ...
    def action_set_demobilize(self):
        ...
        res = super(Task, self).action_set_demobilize()
        return res
