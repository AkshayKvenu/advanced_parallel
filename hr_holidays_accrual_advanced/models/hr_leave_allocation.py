...
class HrLeaveAllocation(models.Model):
    ...
    @api.onchange('employee_id','accrual')
    def set_start_date(self):
        if self.employee_id and self.accrual:
            self.date_from = False
            contract_obj = self.env['hr.contract'].search([('employee_id','=',self.employee_id.id),('state','=','open')])
            if contract_obj:
                self.date_from = str(contract_obj[0].date_start)
     
     

    @api.constrains('holiday_type','holiday_status_id')
    def _onchange_holiday_type(self):  # pragma: no cover
        if self.holiday_status_id.is_annual_vacation:
            if self.holiday_type != 'employee':
                raise UserError(_('leave type should be By employee'))
    ...
    def action_approve(self):
        ...
        allocation_obj = self.env['hr.leave.allocation'].search([('employee_id','=',self.employee_id.id),('holiday_status_id','=',self.holiday_status_id.id),('state','=','validate'),('id','!=',self.id)], limit=1)
        if allocation_obj and allocation_obj.holiday_status_id.is_annual_vacation:
            raise UserError(_('This leave type is already allocated'))
